...
print(sum([int(s) for s in ("".join([i for i in my_string if i != ","and i != "."])).split() if s.isdigit()]))
#Solution 2
print(sum([int(s) for s in [i.rstrip(",").rstrip(",") for i in my_string.split()] if s.isdigit()]))
